      ...
      <tbody>
            ...
            if(mysqli_num_rows($viewOrdersQueryResult)>0){
                while($orders = mysqli_fetch_assoc($viewOrdersQueryResult)){
                        echo "<tr><td class='id'>".$orders['orderid']."</td><td>".$orders['name']."</td><td>".$orders['contact']."</td><td>".$orders['area']."</td><td>".$orders['date']."</td><td>".$orders['status']."</td><td><button class='viewDetails btn btn-light'>View</button></td></tr>";
                }
            }
            else{
